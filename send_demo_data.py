...
from program_reinvent import logs_program_reinvent
...
def get_logs_for_program(prog):
    ...
    if prog['add_logzio_security']:
        for log in temp_logs:
            security_field = copy.deepcopy(logzio_security)
            security_field['ioc']['malicious_ip'] = read_from_nested_dict(log, prog['ip_field'])
            log['logzio_security'] = security_field
    return temp_logs
...
def get_logs_for_time():
    ...
    for prog in logs_program:
        from_time = datetime.time(int(prog['from_time'].split(":")[0]), int(prog['from_time'].split(":")[1]), int(prog['from_time'].split(":")[2]))
        to_time = datetime.time(int(prog['to_time'].split(":")[0]), int(prog['to_time'].split(":")[1]), int(prog['from_time'].split(":")[2]))
        every = prog['every']
